require(['app','bootbox'], function(app,bootbox){
    app.views.asa_cd_anziani_pdl = Backbone.View.extend({
        initialize: function () {
            console.log("initializing asa pdl: ");
        ...
        addRow:function (that) {
           ...
           $(".modal-body").append(        
               ...
               '<div  class="form-group col-lg-12 alle">'+
                   ...
                   '<div class="form-group col-lg-6">'+
                       '<label  >Ruolo</label>'+
                       '<select class="form-control" name="ruolo" id="ruolo"  ></select>'+
                   '</div>'+
                   '<div class="form-group col-lg-6">'+
                       '<label  >Note</label>'+
           ...
           $.each(app.global.nick_array.ruolo, function(i, value) {
            console.log(i,value);
            $('#ruolo').append('<option value="'+value+'">'+value+'</option>');
            
        });
      
           $("#modContratto").validate(); //sets up the validator
           $("input[name=\"ruolo").rules( "add", {
            required: true,
            //number: true,
             minlength: 2,
           ...
           $("#modal").modal('show');
           $('.modal-title').text("Add Piano di lavoro");
           //qui
       ...
       setTab: function (that,my) {//preparo i punti di aggancio
        ...
        switch (that.item) {
            case "pdl":{// Piani di lavoro
                that.$("#boot").empty();
                varForm='<h3> Piani di lavoro <span class="badge">'+my.data.length+' </span></h3><br>'+
                ...
                $.ajax({
                    ...
                    success: function (datap) {
                        ...
                        if ($myData.success) {
                            ...
                            app.global.nick_array.ruolo=$myData.ruolo;
                        }
                        else {
                            bootbox.dialog({
        ...
        destroy_view: function () {
            ...
            app.global.asa_cd_anziani_pdlView=null
        }
    });
return app.views.asa_cd_anziani_pdl;
});