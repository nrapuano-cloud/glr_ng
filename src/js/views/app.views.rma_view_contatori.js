require(['app','bootbox'], function(app,bootbox){
    app.views.rma_view_contatori = Backbone.View.extend({
       
        initialize:function(e){
            
             console.log("initializing rma_view_contatori view")
        ...
        events:{
         //   "click-row.bs.table":"rma_detail",
            ...
            "change #cbEE":"changeCbEE",
            "change #cbAC":"changeCbAC",
            "change #cbGA":"changeCbGA",
        ...
        removeUtenza:function(e, value, row,index){
               console.log("removeUtenza");
                 console.log("id="+row,value,index,e);
                     if (confirm('Vuoi cancellare questa lettura utenza?')) {
                        jsonObj = {};
        ...
        changeCbEE:function(e){
                localStorage.setItem("cbEE", this.$('#cbEE').is(":checked"));
            }, 
        changeCbAC:function(e){
            localStorage.setItem("cbAC", this.$('#cbAC').is(":checked"));
        }, 
        changeCbGA:function(e){
            localStorage.setItem("cbGA", this.$('#cbGA').is(":checked"));
        }, 
       ...
       $.ajax({
        ...
        setRequest:function(e){
            if (this.$('#servizi').is(':empty')){
               ...
               console.log('servizi è vuoto');
                ...
                var $servizio= app.global.tokensCollection.first().get("id_servizio");
                var $selServizio;

                
                   //------------------------prima volta ------------------------------------------------------- 
            $.ajax({
                url:app.global.json_url+ "servizi/list/utenza/"+$person+"/"+$servizio+"/",
                type:'get',
                headers : $headers,
                //data: jsonObj,
                dataType : 'text',
                    ...
                    if ($mydata.success){
                        ...
                        if($mydata.selServizi!==""){
                            $.when(that.$("#servizi").append($mydata.selServizi)).then( function(){
                                ...
                                $bb=$mydata.servizi;
                                $selServizio.append('<option value="0">Tutti i servizi</option>');
          ...
          /*
            ...
            function  callList(that){
                ...
                jsonObj.dataCb=[that.$('#cbEE').is(":checked"),that.$('#cbAC').is(":checked"),that.$('#cbGA').is(":checked")]
                jsonObj = JSON.stringify(jsonObj);
                ...
                success: function (datap) {
                    ...
                    if ($mydata.success){
                      /*  bootbox.dialog({
                        ...
                        bootbox.dialog({
                            title: "View lettura contatori",
                            message: "Errore in View lettura contatori",
                            buttons: {
                                main: {
                                    label: that.language.label_button,
                                    className: "btn btn-danger",
                                    callback: function() {
                                        $("body").removeClass("modal-open");
                                       //  app.routers.rmaRouter.prototype.rma();
                                    }
                                }
                            }
                        });
        ...
        hrTable:function(my){
            ...
            $.each( my.data, function( key, value1 ){
                if(typeof(value1["data_lettura"]) !== "undefined" && value1["data_lettura"] !== null){    
                    value1["dateT"]='<span>'+moment(value1["data_lettura"]).format('YYYYMMDD')+'</span>'+moment(value1["data_lettura"]).format('DD/MM/YYYY');
                    value1["data_lettura"]=moment(value1["data_lettura"]).format('DD/MM/YYYY');
                }
             });   
            this.$('#lblCounter').text('Trovate '+my.data.length+' letture utenze');
        ...
        render:function(){
            ...
            if(localStorage.getItem("cbEE")!=="" && localStorage.getItem("cbEE")!==null){
             var $AA=localStorage.getItem("cbEE")==="true"?true:false;
               this.$('#cbEE').prop('checked', $AA);
               
            }
            if(localStorage.getItem("cbAC")!=="" && localStorage.getItem("cbAC")!==null){
                  var $AA=localStorage.getItem("cbAC")==="true"?true:false;
               this.$('#cbAC').prop('checked', $AA);
            }
            if(localStorage.getItem("cbGA")!=="" && localStorage.getItem("cbGA")!==null){
                var $AA=localStorage.getItem("cbGA")==="true"?true:false;
               this.$('#cbGA').prop('checked', $AA);
            }
                
            this.setRequest();
            that=this;
            window.actionEvents = {
                'click .update': function ($element, value, row,index) {
                    app.global.nick_array.id=row.id;
                    
                    app.global.breadcrumb.push({
                       breadcrumb: '<li class="breadcrumb-item active">'+row.servizio+ " lettura utenza "+row.utenza+'</a></li>'
                    ...
                    app.global.rma_view_contatoriView.destroy_view();  
                    app.routers.rmaRouter.prototype.rma_new_contatori({});
                  
                   
                },
                'click .removeUtenza': function (e, value, row,index) {
                    console.log("id="+row);
                    if (confirm('Sei sicuro di voler eliminare questa lettura?')) {
                        ...
                        $.ajax({
                            ...
                            success: function (datap) {
                                $mydata =JSON.parse(datap);
                                bootbox.dialog({
                                    title: "Delete lettura contatori",
                                    message: "Delete lettura contatori OK",
                                    buttons: {
                                        main: {
                                            label: "OK",
                             ...
                             error: function () {
                                bootbox.dialog({
                                    title: "Delete lettura contatori",
                                    message: "Errore in Delete lettura contatori",
                                    buttons: {
                                        main: {
                                            label: "Errore",
                                            className: "btn btn-danger",
...
return app.views.rma_view_contatori;
    });
