...
require(['app.min',"he.js","bootbox.all.min.js","bootstrap-select.min.js"], function(app,he,bootbox,selectpicker){
    ...
    app.functions['rfa_fornitori'] = function (that,$row){
  ...
  //el.innerHTML = 'Hello World!';
          ...
          //-------------------------------------event-------------------------------------------------------------------
              ...
              that.$('#categorie').click(function () {
                  ...
                  app.functions.rfa_fornitori_categorie(that,$row);
              }); 
              that.$('#prodotti').click(function () {
                  console.log("prodotti");
                  console.log(that);
                   console.log($row);
                   app.functions.rfa_fornitori_prodotti(that,$row.id);
              }); 
    ...
    app.functions['rfa_fornitori_categorie'] =function (that,$row){
    ...
    app.functions['rfa_fornitori_prodotti'] = function (that,$id){ 
      //  that=this;
    ...
    app.functions['rfa_tipologie_prodotto_servizio'] = function (that,$row){
    ...
    app.functions['rfa_tipologie_fornitori_acquisto_generale'] =function (that,$row){
    ...
    app.functions['rfa_servizi'] = function(that,$row){
        var API_URL = app.global.json_url;
        var $headers = {
                "Authorization" : "Bearer " + app.global.tokensCollection.first().get("auth").token,
                "username" : app.global.tokensCollection.first().get("username"),
    ...
    that.$('#btnSub').click(function(e) {
      ...
      window.actionEvents = {
            'click .updateForn': function (e, value, row) {
               ...
               app.functions.rfa_servizi_prodotti(that,row.id_fornitore);//idPro id fornitore
            },
    ...
    app.functions['rfa_servizi_prodotti'] =function (that,$row){
    ...
    app.functions['rfa_banche'] =function (that,$row){
        var $headers = {
                "Authorization" : "Bearer " + app.global.tokensCollection.first().get("auth").token,
                "username" : app.global.tokensCollection.first().get("username"),
                "lang" : app.global.languagesCollection.at(0).get("lang"),
...
//-----------------------referenti banche----------------------------------------------------------- 
    ...
    app.functions['rma_pmaTipologie'] = function(that,$row){
        var API_URL = app.global.json_url;
        var $headers = {
                "Authorization" : "Bearer " + app.global.tokensCollection.first().get("auth").token,
                "username" : app.global.tokensCollection.first().get("username"),
    ...
    app.functions['utenze_fornitori'] = function (that,$row){
        var isNew=app.global.nick_array.isNew;  
         var API_URL = app.global.json_url + 'types/';
       var $iEmail=0;
       var $iTel=0;
                       ...
                       '<form class="registrationForm" id="registrationForm" method="post">'+
                           ...
                           '<div class="row">'+
                              '<div class="form-group col-lg-4">'+
                                   ...
                                   '<select  name="tipo" id="tipo"  class="form-control tiposel" multiple></select>'+
                       ...
                       that.$("#registrationForm").validate({
                           ...
                           messages: {
                               name: {
                                 ...
                                 tipo: {
                                 required: "Perfavore inserisci la tipologia di fornitura"
               ...
               $.ajax({
                  ...
                  if(isNew){
                       ...
                       function  tipo(){    
                           ...
                           var jsonObj = {};
                           jsonObj.action = "list";
                           jsonObj.type = "utenze_fornitori_tipo";
                           jsonObj.person = app.global.tokensCollection.first().get("id_person");
                           jsonObj = JSON.stringify(jsonObj);
                           ...
                           $.ajax({
                               ...
                               dataType : 'text',
                                success: function (json) {
                                   ...
                                   var lista   = [];
                                   lista.push('<option value=""></option>');
                                   $.each($aa, function(i, value) {
                                       lista.push('<option  value="' + value + '">' +value + '</option>');
                                   });
                                   $selTipo.html(lista.join(''));
                                  $selTipo.selectpicker('refresh');
                                   if( !isNew){//update
                                       lista   = [];
                                       $.each(($row.tipo), function(i, value) {
                                           console.log(i,value);
                                           lista.push(value);
                                       });
                                        console.log(lista);
                                       $selTipo.selectpicker('val', lista);
                                      $selTipo.selectpicker('render');
